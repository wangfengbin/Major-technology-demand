		...
		<div class="row-fluid">
			...
			<center>
				...
				<c:if test="${page.currPage!=1 }">
					<a href='${pageContext.request.contextPath}/NeedServlet?method=fenye&currPage=1'>[首页]  </a>
					<a href='${pageContext.request.contextPath}/NeedServlet?method=fenye&currPage=${page.currPage-1}'>[上一页]</a>
				</c:if>
				...
				<c:forEach begin="1" end="${page.totalPage }" var="n">
					...
					<!-- 若不是当前页 可点 -->
					<c:if test="${page.currPage != n }">
						<a href="${pageContext.request.contextPath}/NeedServlet?method=fenye&currPage=${n}">${n }</a>
					</c:if>
				</c:forEach>
				
				<!-- 若是最后一页 尾页和下一页不展示 -->
				<c:if test="${page.currPage!=page.totalPage }">
					<a href='${pageContext.request.contextPath}/NeedServlet?method=fenye&currPage=${page.currPage+1}'>[下一页]</a>
					<a href='${pageContext.request.contextPath}/NeedServlet?method=fenye&currPage=${page.totalPage}'>[尾页]  </a>
				</c:if>
